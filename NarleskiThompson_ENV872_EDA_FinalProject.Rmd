---
title: "Fish Tissue Contamination Trends in the Great Lakes"
subtitle: "GitHub Repsitory: https://github.com/j-narleski/NarleskiThompson_ENV872_EDA_FinalProject"
...
laketheme <- theme(
  ...
  plot.title = element_text(
    ...
    face = "bold"),
  axis.title = element_text(size = 10, color = "darkcyan", face = "bold"),
...
# 2010 data
fattyacids_2010_raw <- read.csv("Data/RawData/2010_FattyAcids_Raw.csv")
mercury_2010_raw <- read.csv("Data/RawData/2010_Mercury_Raw.csv")
PCB_2010_raw <- read.csv("Data/RawData/2010_PCBs_Raw.csv")
PFA_2010_raw <- read.csv("Data/RawData/2010_PFAs_Raw.csv")

# 2015 data
fattyacids_2015_raw <- read.csv("Data/RawData/2015_FattyAcids_Raw.csv")
mercury_2015_raw <- read.csv("Data/RawData/2015_Mercury_Raw.csv")
PCB_2015_raw <- read.csv("Data/RawData/2015_PCBs_Raw.csv")
PFA_2015_raw <- read.csv("Data/RawData/2015_PFAs_Raw.csv")

# 2020 data
fattyacids_2020_raw <- read.csv("Data/RawData/2020_FattyAcids_Raw.csv")
mercury_2020_raw <- read.csv("Data/RawData/2020_Mercury_Raw.csv")
PCB_2020_raw <- read.csv("Data/RawData/2020_PCBs_Raw.csv")
PFA_2020_raw <- read.csv("Data/RawData/2020_PFAs_Raw.csv")

```



```{r, out.width="70%", out.height="70%", fig.align='center', echo = FALSE, message = FALSE}
knitr::include_graphics(here("Data/ProcessedData", "fishimage_PaulVecsei.jpg"))
```
<div style="text-align: center; color: darkgray;">
  Herring in the Great Lakes. All Rights to Paul Vecsei (9)
</div>
...
<ol><b>
  ...
  <li>How do contaminants vary depending on if fish is an open-sea or bottom dweller (niche)? </li>
</b></ol>
...
<br>
<ol>
  <li>Sites are randomly selected along U.S. coasts of the Great Lakes</li>
  <li>Crews collect fish at all NCCA sites, yielding a minimum total of 300g total mass per target species</li>
  <li>Fish are identified, measured, labelled, frozen, and brough to a lab </li>
  <li>Specimens are cleaned to prevent contamination with non-marine chemicals</li>
  <li>Fish are filleted (scales are removed, but skin and belly are left intact)</li>
  <li>A variety of lab procedures are performed in accordance with EPA guidelines to determine the lipid content, concentrations of metals, pesticides, and PCBs in each fish</li>
</ol>
<br>
The Environmental Protection Agency has conducted periodic studies over the past 15 years assessing the coastal conditions of marine and the Great Lakes shores. We focus on data from 2010, 2015, and 2020. The basic structure of each raw data file is described below (Table \@ref(tab:columm-description-table) ). 

```{r columm-description-table, echo=FALSE, message=FALSE}

column_descriptions <- data.frame(
  `Column Name` = c(
    "EPA Region", "State", "Lake Name", "Site ID", 
    "EPA Sample ID", "Species ID", "Scientific Name", 
    "Common Name", "Family", "Niche", "Analyte", 
    "Abbreviation", "CAS Number", "Amount", 
    "Unit.1", "Unit.2", "MDL", "QL", 
    "Lab Flag", "SCC Code", "Comment"
  ),
  Description = c(
    ...
    "Individual numeric code assigned to each fish species",
    "Taxonomic name of each species",
    "Taxonomic family of each species",
    ...
    "The abbreviation for the contaminant being measured",
    "A unique identifier assigned to chemical substances by the Chemical Abstracts Service",
    "Amount of contaminant",
    "The original unit the chemical was measured in ('ng/g' for Mercury, PFA, PCB; 'mg/g' for Fatty Acids)",
    "The amount of chemical converted to common units ('ppb' for Mercury, PFA, PCB; 'ug/g' for Fatty Acids)",
    "Method Detection Limit - the minimum concentration of a substance that can be reliably detected and reported with 99% confidence",
    "Quantification Limit - the lowest concentration that can be reliably measured within a certain level of precision and accuracy",
    "Unique identifier for the lab used",
    "Code for the container the fish was stored in between catching and processing",
    "Comment on results input by researcher"
  )
)
...
variable_descriptions <- data.frame(
  ...
  Description = c(
    ...
    "Column added manually to assign sample year to each result"
  )
)
...
```{r pfas-table, echo=FALSE, message=FALSE}

pfas <- data.frame(
  PFA = c(
     "Perfluorooctanoate", "Perfluorooctanesulfonate", 
     "Perfluorohexane sulfonate", "Perfluorononanoate", 
     "Hexafluoropropylene oxide dimer acid" 
     ),
   Abbreviation = c("PFOA", "PFOS", "PFHxS", "PFNA", "HFPO-DA"),
   `Maximum Contaminant Level (MCL)` = c("4.0 ppt", "4.0 ppt", "10 ppt", 
                                         "10 ppt", "10 ppt")
)
names(pfas) <- c("PFA", "Abbreviation", "Maximum Contaminant Level (MCL)")

# Generate the table
kable(
  pfas, 
  caption = "Types of PFAs, their abbreviations, and the maximum contaminant level allowed in drinking water."
)
...
names(fatty_acids) <- c("Fatty Acid", "Type", "Abbreviation")
...
"~/FinalProject/NarleskiThompson_ENV872_EDA_FinalProject/Data/ProcessedData/2010_Mercury_Processed.csv")
...
write.csv(mercury_2015_processed, "~/FinalProject/NarleskiThompson_ENV872_EDA_FinalProject/Data/ProcessedData/2015_Mercury_Processed.csv")
...
"~/FinalProject/NarleskiThompson_ENV872_EDA_FinalProject/Data/ProcessedData/2020_Mercury_Processed.csv")
...
write.csv(PFA_2010_processed, "~/FinalProject/NarleskiThompson_ENV872_EDA_FinalProject/Data/ProcessedData/2010_PFAs_Processed.csv")
...
write.csv(PFA_2015_processed, "~/FinalProject/NarleskiThompson_ENV872_EDA_FinalProject/Data/ProcessedData/2015_PFAs_Processed.csv")
...
write.csv(PFA_2020_processed, "~/FinalProject/NarleskiThompson_ENV872_EDA_FinalProject/Data/ProcessedData/2020_PFAs_Processed.csv")
...
write.csv(PCB_2010_processed, "~/FinalProject/NarleskiThompson_ENV872_EDA_FinalProject/Data/ProcessedData/2010_PCBs_Processed.csv")
...
write.csv(PCB_2015_processed, "~/FinalProject/NarleskiThompson_ENV872_EDA_FinalProject/Data/ProcessedData/2015_PCBs_Processed.csv")
...
write.csv(PCB_2020_processed, "~/FinalProject/NarleskiThompson_ENV872_EDA_FinalProject/Data/ProcessedData/2020_PCBs_Processed.csv")
...
write.csv(fattyacids_2010_processed, "~/FinalProject/NarleskiThompson_ENV872_EDA_FinalProject/Data/ProcessedData/2010_FattyAcids_Processed.csv")
...
write.csv(fattyacids_2015_processed, "~/FinalProject/NarleskiThompson_ENV872_EDA_FinalProject/Data/ProcessedData/2015_FattyAcids_Processed.csv")
...
write.csv(fattyacids_2020_processed, "~/FinalProject/NarleskiThompson_ENV872_EDA_FinalProject/Data/ProcessedData/2020_FattyAcids_Processed.csv")
...
#site map
sampling_sites <- read_csv("Data/RawData/2020sitelocationsv6.csv")
...
states_sf <- st_read(here('Data/RawData/cb_2018_us_county_20m.shp'), quiet=TRUE) %>%
...
```{r mercury-exp-lake, echo=FALSE, fig.align='center', fig.cap='Exploratory analysis of mean mercury levels in each of the Great Lakes', warning=FALSE, message=FALSE}
...
```{r mercury-exp-fish, echo=FALSE, fig.align='center', fig.cap='Exploratory analysis of mean mercury levels in each sampled fish species', warning=FALSE, message=FALSE}
...
```{r mercury-exp-niche, echo=FALSE, fig.align='center', fig.cap='Exploratory analysis of mean mercury levels by fish niche', warning=FALSE, message=FALSE}
...
```{r pcb-exp-lake, echo=FALSE, fig.align='center', fig.cap='Exploratory analysis of mean PCB levels in each of the Great Lakes', warning=FALSE, message=FALSE}
PCB_by_lake <- ggplot(
...
```{r pcb-exp-fish, echo=FALSE, fig.align='center', fig.cap='Exploratory analysis of mean PCB levels in each sampled fish species', warning=FALSE, message=FALSE}
...
```{r pcb-exp-niche, echo=FALSE, fig.align='center', fig.cap='Exploratory analysis of mean mercury levels by fish niche', warning=FALSE, message=FALSE}
...
```{r fatty-exp-fish, echo=FALSE, fig.align='center', fig.cap='Exploratory analysis of mean fatty acid levels by fish species', warning=FALSE, message=FALSE}
...
```{r pfa-exp-lake, echo=FALSE, fig.align='center', fig.cap='Exploratory analysis of total PFA levels by lake', warning=FALSE, message=FALSE}
...
```{r pfa-exp-fish, echo=FALSE, fig.align='center', fig.cap='Exploratory analysis of mean PFA levels by fish species', warning=FALSE, message=FALSE}
# Combined by fish species
PFA_byfish_combined <-
...
```{r pfa-exp-niche, echo=FALSE, fig.align='center', fig.cap='Exploratory analysis of mean PFA levels by fish niche', warning=FALSE, message=FALSE}
# Combined by niche
...
```
<br> 
This process is repeated for PCBs. The same 14 fish as mercury had data available for all three years (Figure \@ref(fig:PCB-fish-trend-plot)). Of all 14 fish, only one presented a p-value indicating a significant relationship between PCB level and year for that species. **White Perch** had a p-value of 0.04507 (p<0.5) and an adjusted R-squared value of 0.99.  
<br>
...
## Question 3: How do contaminants vary depending on if fish is an open-sea or bottom dweller (niche)?
<br>
To continue our investigation of what may make certain fish more susceptible to contamination, another factor aside from species is the fish niche. In NCCA data collection, there were two options for fish niche: "BD" (Benthic-Demersal- i.e. bottom dweller) and"P" (Pelagic- i.e. open sea). We compare the mean contaminant amounts between BD and P fish across each sampling year using an **ANOVA**. The results of the ANOVA are summarized in Table \@ref(tab:contaminant-anova-niche-table) . 

```{r niche-anovas, echo=FALSE, warning=FALSE, message=FALSE, include = FALSE}
#Mercury 2010
mercury_2010_processed$Niche <- as.factor(mercury_2010_processed$Lake)
mercury_byniche_2010_AOV <- aov(Amount ~ Niche, data = mercury_2010_processed)
summary(mercury_byniche_2010_AOV)
  #p = 8.74e-05 (SIG)

#Mercury 2015
mercury_2015_processed$Niche <- as.factor(mercury_2015_processed$Niche)
mercury_byniche_2015_AOV <- aov(Amount ~ Niche, data = mercury_2015_processed)
summary(mercury_byniche_2015_AOV)
  #p = 1.03e-06

#Mercury 2020
mercury_2020_processed$Niche <- as.factor(mercury_2020_processed$Niche)
mercury_byniche_2020_AOV <- aov(Amount ~ Niche, data = mercury_2020_processed)
summary(mercury_byniche_2020_AOV)
  #p =  0.000249 (SIG)

#PCB 2010
PCB_2010_processed$Niche <- as.factor(PCB_2010_processed$Niche)
PCB_byniche_2010_AOV <- aov(Amount ~ Niche, data = PCB_2010_processed)
summary(PCB_byniche_2010_AOV)
  #p = 0.169 (NOT SIG)

#PCB 2015
PCB_2015_processed$Niche <- as.factor(PCB_2015_processed$Niche)
PCB_byniche_2015_AOV <- aov(Amount ~ Niche, data = PCB_2015_processed)
summary(PCB_byniche_2015_AOV)
  #p = 0.00111 (SIG)

#PCB 2020
PCB_2020_processed$Niche <- as.factor(PCB_2020_processed$Niche)
PCB_byniche_2020_AOV <- aov(Amount ~ Niche, data = PCB_2020_processed)
summary(PCB_byniche_2020_AOV)
  #p = 0.0138 (SIG)

#PFA 2010
PFA_2010_processed$Niche <- as.factor(PFA_2010_processed$Niche )
PFA_byniche_2010_AOV <- aov(Amount ~ Niche, data = PFA_2010_processed)
summary(PFA_byniche_2010_AOV)
  #p = 0.0799 (NOT SIG)

#PFA 2015
PFA_2015_processed$Niche <- as.factor(PFA_2015_processed$Niche )
PFA_byniche_2015_AOV <- aov(Amount ~ Niche, data = PFA_2015_processed)
summary(PFA_byniche_2015_AOV)
  #p = 0.00496 (SIG)

#PFA 2020
PFA_2020_processed$Niche <- as.factor(PFA_2020_processed$Niche)
PFA_byniche_2020_AOV <- aov(Amount ~ Niche, data = PFA_2020_processed)
summary(PFA_byniche_2020_AOV)
  #p = 0.176 (NOT SIG)

```

```{r contaminant-anova-niche-table, echo=FALSE, message=FALSE}
contaminant_anova_niche <- data.frame(
  Contaminant = c(
    "Mercury", "Mercury", "Mercury", 
    "PCB", "PCB", "PCB", 
    "PFA", "PFA", "PFA"
  ...
  Year = c(2010, 2015, 2020, 2010, 2015, 2020, 2010, 2015, 2020),
  `F Value` = c(6.397,25.97,14.03,1.912,13.52,6.196,3.07,7.912,1.83),
  `P-value` = c(8.74e-05,1.03e-06,0.000249,0.169,0.000329,0.0138,0.0799,0.00496,0.176),
  `Significant? (p < 0.05)` = c(
    "<span style='color:green'>YES</span>", "<span style='color:green'>YES</span>", 
...
names(contaminant_anova_niche) <- c("Contaminant", "Year", "F Value", "P-value", "Significant? (p<0.05)")

kable(
  contaminant_anova_niche, 
  caption = "ANOVA results of mean contaminant levels found in fish samples from both Niches from 3 sampling years.", 
  escape = FALSE
)
```
<br>
Since there are only two means to compare, a Tukey test is not necessary. All ANOVAs that indicate a difference between means indicate that the mean level of contamination of Benthic and Pelagic fish are significantly different. Per the results of our prior exploratory analysis, all Pelagic fish for all contaminants had higher levels of contamination. 
<br>
<br>
For Mercury, the p-values for all years (p=8.74e-05, p=1.003e-06,2.49e-04; p<0.05) indicate that Pelagic fish had significantly higher contamination than Benthic fish. For PCBs, only 2015 and 2020 were significant (p=3.29e-04, p=1.38e-02; p<0.05); for PFAs, there was only one year for which the difference was significant (p=4.96e-03; p<0.05). 
\newpage

# Summary and Conclusions
<br>
The data collected through the NCCA surveys provide crucial insight into the contamination of fish tissue in the Great Lakes. Using this data, we were able to determine that the levels of contaminants are not evenly distributed across the Great Lakes, as there is significant differences between mean contaminant levels in most cases. Further, certain lakes tend to have contaminant means that are not statistically different from each other (Lakes Erie, Huron, and Michigan), while Lakes Ontario and Superior tend to be statistically different than other lakes. This may be a result of geographic differences in the lakes, or national-level regulations that apply to the lakes that are shared between the U.S. and Canada.
The variance between lakes indicates that broad and comprehensive management plans for all of the lakes may not be the best solution for addressing this contamination. Management plans should be tailored to each individual lake, with special attention taken towards the lakes that tend to behave differently. Since the Great Lakes do act as drinking water sources for many communities, it is important that contaminant levels are continuously monitored and mitigated when necessary.
<br>
<br> 
For our second research question, we found that there are certain fish species that have significnat trends in contaminantion over time, but the number of fish species that do not have significant trends outnumber the ones that do. For Mercury, there were no significant trends, which may be attributed to the capacity for non-discriminatory contamination of this chemical. Mercury may be a threat to all fish, as there may not be any biological differences that can protect certain fish from it. We did find significant relationships between year and White Perch (PCBs), Brown Trout (PFAs), and Walleye (PFAs). For these species, the trends were downward- that is, the amount of contamination is decreasing over time. While more data would be required to support this claim, it is possible that these species are adapting in some way to reduce their contamination levels, potentially through changes in habitat, feeding behaviors, or physiological mechanisms. Continued analysis is needed, especially in consideration of recreational fishing and consumption of fish that would pass on contaminants to the general public.
<br>
<br> 
Lastly, we found that, in most cases sampled, Pelagic fish had significantly higher means of contamination. Pelagic fish primarily reside and hunt in open waters; recognizing that these fish have higher levels of contaminants may point researchers towards further investigations into environmental factors that contribute to this contamination. Specifically, it may suggest a closer examination of aquatic pollution sources such as industrial discharge, agricultural runoff, or plastic waste (specifically for PFAs) that could be affecting these species.Further, pelagic fish tend to be predators, which indicates the potential consequences of biomagnification, where contaminants increase in concentration as they move up the food chain [8].
<br>
<br> 
Processing this data revealed a lot about data collected in the field, and the importance of formatting consistency for long-term data sets. While some trends and relationships were found to be significant in our analysis, more data points are required for more confidence. This should be motivation to the EPA to continue the NCCA program, as the current sample size is very small. However, if the trends we found are any indication, the contamination of fish should absolutely continue to be monitored over time, especially given the potential long-term impacts on aquatic ecosystems and human health. 
...
<br>
[5] Contaminants in Fish. (2021, April 5). Indiana Department of Health. https://www.in.gov/health/eph/fish-consumption-advisory/contaminants-in-fish/
<br>
[6] US EPA, O. (2021, November 16). Per- and Polyfluoroalkyl Substances (PFAS) [Reports and Assessments]. https://www.epa.gov/sdwa/and-polyfluoroalkyl-substances-pfas
<br>
[7] Excess omega-3 fatty acids could lead to negative health effects. (n.d.). Oregon State University College of Health. Retrieved December 12, 2024, from https://synergies.oregonstate.edu/2013/excess-omega-3-fatty-acids-lead-negative-health-effects/
<br>
[8] US Department of Commerce, N. O. and A. A. (n.d.). What are pelagic fish? Retrieved December 12, 2024, from https://oceanservice.noaa.gov/facts/pelagic.html
<br>
[9] Fighting for Our Fisheries. (n.d.). The Nature Conservancy. Retrieved December 12, 2024, from https://www.nature.org/en-us/about-us/where-we-work/priority-landscapes/great-lakes/stories-in-the-great-lakes/great-lakes-fisheries/
